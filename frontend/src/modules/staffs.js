...
// ==========================================
// START: initStaffs
// Initializes the Staffs SPA routing system by attaching hash listeners and loading the initial route.
// ==========================================
export function initStaffs() {
  const container = document.getElementById('staffs-content-container') || document.getElementById('users-content-container');
  if (!container) return; // We are not on the Staffs page

  // Initial load
  handleStaffRoute();

  // Listen for hash changes (when user clicks sidebar links)
  window.addEventListener('hashchange', handleStaffRoute);
}
// ==========================================
// END: initStaffs
// ==========================================
...
export const initUsers = initStaffs;

// ==========================================
// START: handleStaffRoute
// Dynamically fetches and renders the staff sub-view (owner, cashiers, inventorym) based on URL hash.
// ==========================================
async function handleStaffRoute() {
  const container = document.getElementById('staffs-content-container') || document.getElementById('users-content-container');
  if (!container) return;
  ...
  try {
    const response = await fetch(`/pages/users/client/components/staffs/${component}.html`);
    if (response.ok) {
      ...
      // Re-bind theme toggles if any were in the new HTML
      initTheme();
    ...
    console.error('Failed to load staff component:', error);
    container.innerHTML = `
